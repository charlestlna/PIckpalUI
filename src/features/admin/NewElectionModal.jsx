...
const DEPARTMENTS = ["SSC", "CLA", "CED", "CHM", "CCS", "CBA", "CCJE"];
const DEPARTMENT_LABELS = {
  SSC: "SSC - All departments",
  CLA: "CLA",
  CED: "CED",
  CHM: "CHM",
  CCS: "CCS",
...
const NewElectionModal = ({ onClose, onCreated }) => {
  ...
  const valid = name.trim() && startDate && endDate && endDate >= startDate && positions.length > 0;
  ...
  const handleCreate = async () => {
    ...
    try {
      await onCreated?.({ name, department, startDate, endDate, positions });
      setSaving(false);
    ...
    <div className="overlay-centered" onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      <div className="modal-centered">
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
          <div>
            <h2 style={{ fontFamily:"var(--font-display)", fontSize:22, color:"var(--navy)" }}>New Election</h2>
            <p style={{ fontSize:13, color:"var(--gray-500)", marginTop:3 }}>Configure details and positions</p>
          </div>
        ...
        <div style={{ marginBottom:16 }}>
          <label className="label">Eligible Department</label>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
          ...
          <div>
            ...
            <input className="input-field" type="date" value={endDate} onChange={e => setEnd(e.target.value)} min={startDate} />
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:10 }}>
            {positions.map((p,i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(13,148,136,0.08)", border:"1px solid rgba(13,148,136,0.25)", borderRadius:"var(--radius-full)", padding:"5px 12px" }}>
